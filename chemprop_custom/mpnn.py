...
class MPNN(pl.LightningModule):
    ...
    def fingerprint(
        self, bmg: BatchMolGraph, V_d: Tensor | None = None, X_d: Tensor | None = None, text_inputs: Optional[list[str]] = None
    ) -> Tensor:
        """Calculate learned fingerprints for the input molecules, including text embeddings."""
        ...
        if text_inputs is not None:
            ...
            with torch.no_grad():
                text_embeddings = self.text_encoder(
                    **text_tokens
                ).last_hidden_state.mean(dim=1)  # Mean pooling

            # Combine molecular and text embeddings along feature dimension
            H = torch.cat((H, text_embeddings), dim=1) # 
    ...
    def forward(
        ...
        """Generate predictions for the input molecules/reactions with text inputs."""
        return self.predictor(self.fingerprint(bmg, V_d, X_d, text_inputs))
